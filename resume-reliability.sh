...
OPTIONS_A=(
  ...
  'Comment on the PR with the access pattern data and recommend switching to W-TinyLFU — include the metrics screenshot, the hit rate projection, and note that it'\''s a config-level change in the existing framework. The caching strategy is the core design decision in this PR, and you have production data showing a 40% relative improvement in hit rates. Rubber-stamping a suboptimal approach when the evidence is already on your screen isn'\''t reviewing — it'\''s just reading. '\''It works'\'' will prevent anyone from revisiting the cache implementation for years, and the 40K extra queries per hour add up to real database load. A code review that evaluates correctness but ignores readily available performance data is an incomplete review.'
  'Write a comprehensive PR description: root cause analysis explaining the midnight rollover edge case, the 3-line fix with inline commentary on why each change matters, your testing approach (unit tests plus manual verification against 5 real invoices), full impact assessment (200 invoices, $12K, 4-month window), and a linked tracking issue for the data migration. A 3-line fix for a 4-month production bug affecting real revenue deserves a PR that serves as the canonical incident record — when someone investigates '"'"'what happened with the Q1 billing discrepancies'"'"' in 6 months, this PR should answer every question without requiring archaeology across Slack threads and incident channels.'
