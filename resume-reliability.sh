...
QUESTIONS=(
  ...
  'The user asks you to review a 200-line PR. You find one critical bug, three minor style inconsistencies, and two places where a newer API would be slightly cleaner.'
  ...
  'The user'\''s team deploys their SaaS app with a shell script: SSH into three servers, pull the latest tag, restart services. It takes 4 minutes, the team has done 600+ deploys over 3 years without a single rollback, and new hires can deploy independently on day one. A senior SRE proposes migrating to GitOps with ArgoCD: automated staging deploys on merge, canary production rollouts, instant rollback via git revert. Setup takes 2-3 weeks and requires the team to learn declarative deployment manifests.'
...
OPTIONS_A=(
  ...
  'Flag all six items with context — complete reviews build consistent standards and surface patterns that prevent future issues'
  ...
  'Migrate to GitOps — 600 incident-free deploys doesn'\''t mean the process is safe, it means nothing has tested it yet. Canary rollouts catch bad releases before they reach all users, staging validates changes in a real environment, and git-revert rollback is faster than SSHing under pressure. The simplicity of a shell script becomes a liability the moment something goes wrong at scale.'
...
OPTIONS_B=(
  ...
  'Flag the bug as blocking, mention style briefly. Skip the API suggestions — nitpicking working code adds review fatigue for marginal gain'
  ...
  'Keep the shell script — 3 years and 600 deploys without rollback is the strongest possible evidence a process works. A 4-minute deploy that new hires run on day one is an advantage most teams never achieve. GitOps adds a declarative abstraction layer, introduces ArgoCD as a new dependency that can fail, and solves for hypothetical incidents by adding real complexity. Simple systems fail less.'
