...
const QUESTIONS = {
  en: [
    ...
    {q:"You're writing a technical spec for a feature that two teams will implement in parallel. The feature is real-time order status notifications via WebSocket. Both the backend team (building the server) and the mobile team (building the client) need to integrate without blocking each other.",a:"Specify everything: WebSocket message schema with exact field names, types, and validation rules; reconnection protocol with exponential backoff intervals and max retry count; all 7 notification event types with complete payload examples; error codes with handling guidance for each failure mode; and a sequence diagram for the full connection lifecycle. Both teams can implement independently without sync meetings, and the spec becomes the integration test contract — if the implementations don't match the spec, the implementation is wrong. The 2 days spent now prevents the 2 weeks of integration debugging when each team made different assumptions about message formats, reconnection behavior, and error semantics.",b:"Specify the interface contract: WebSocket endpoint URL, authentication handshake, and the 3 most common notification event types with example payloads. Leave reconnection timing, timeout values, and edge-case handling to each team — the mobile client has different constraints than the web client, and specifying behavior you haven't built yet is guessing in formal notation. Over-specified interfaces create coupling: when implementation reality diverges from pre-built spec details, teams debate whether to update the spec or deviate from it, instead of shipping. Define the contract, ship the core events, learn from the first integration, then document the rest from reality."},
    {q:"You've just deployed a new version of your team's API service. The release changes 2 of the service's 15 endpoints — adding a new filter parameter to the search endpoint and increasing the rate limit on the bulk export endpoint. Your team's deployment pipeline ran green (build, unit tests, integration tests all passed). The service handles internal traffic from 3 downstream consumers.",a:"Run the full verification checklist: smoke test all 15 endpoints, compare response times against the pre-deployment baseline, verify structured logs are flowing to the aggregator, confirm metrics are reporting to the monitoring dashboard, and watch error rates for 30 minutes before marking the deployment complete. The 13 unchanged endpoints still run on the new container image with updated dependencies and potentially different runtime characteristics — a config drift, a memory limit, or an environment variable change can affect endpoints that didn't change in the code. Catching a regression during a 30-minute deployment soak is 10x cheaper than discovering it from a downstream consumer's escalation 4 hours later.",b:"Verify the 2 changed endpoints and their dependency paths, then check the global error rate and latency dashboards for anomalies — a 5-minute check. The 13 unchanged endpoints ran through identical integration tests on the same container image 20 minutes ago in CI. Manually re-testing them in production is testing the deployment pipeline, not the code change — and that's what pipeline health checks are for. The real risk calculus: teams that turn verification into a 40-minute ceremony deploy less frequently, batch more changes per release, and increase the blast radius of each deployment. A 5-minute targeted verification with continuous monitoring catches regressions faster than a 40-minute manual soak, because the monitoring runs 24/7 and the manual checklist runs once."},
  ...
  zh: [
    ...
    {q:"你在写一个两个团队需要并行实现的功能技术规格书。功能是通过 WebSocket 实现实时订单状态通知。后端团队（构建服务端）和移动端团队（构建客户端）需要在不互相阻塞的情况下各自完成集成。",a:"全面规定：WebSocket 消息 schema 精确到字段名、类型和校验规则；重连协议包含指数退避间隔和最大重试次数；所有 7 种通知事件类型都附完整 payload 示例；每种错误模式都有错误码和处理指南；加上完整的连接生命周期时序图。两个团队可以完全独立实现，不需要同步会议，spec 就是集成测试契约——实现和 spec 对不上，改的是实现不是 spec。现在花 2 天写清楚，省的是之后两个团队对消息格式、重连行为、错误语义理解不一致时 2 周的集成调试。",b:"只规定接口契约：WebSocket 端点 URL、认证握手流程、3 种最常见通知事件类型附 payload 示例。重连时机、超时值、边界情况处理留给各团队自行决定——移动端和 Web 端约束不同，在没写过代码之前就规定重连策略是用正式文档格式在猜。过度规定的接口制造耦合：实现和 spec 细节不一致时，两个团队陷入\"改 spec 还是偏离 spec\"的讨论，而不是在写代码。定义好契约，上线核心事件，从第一次真实集成中学习，然后基于现实补充文档。"},
    {q:"你刚部署了团队 API 服务的新版本。这次发布修改了服务 15 个端点中的 2 个——搜索端点新增了一个过滤参数，批量导出端点提高了速率限制。团队的部署流水线全部通过（构建、单元测试、集成测试均为绿色）。该服务处理来自 3 个下游消费方的内部流量。",a:"执行完整验证清单：对所有 15 个端点进行冒烟测试，将响应时间与部署前基线对比，验证结构化日志正确流入聚合器，确认指标正在上报到监控面板，然后观察错误率 30 分钟再标记部署完成。那 13 个未改动的端点同样运行在新容器镜像上，使用更新后的依赖，运行时特性可能不同——配置漂移、内存限制、环境变量变更都可能影响代码层面没改的端点。在 30 分钟部署观察期发现回归，成本是下游消费方 4 小时后升级报告的十分之一。",b:"验证 2 个改动端点及其依赖路径，然后检查全局错误率和延迟面板是否有异常——5 分钟搞定。那 13 个未改动的端点 20 分钟前已经在 CI 中用相同容器镜像跑过完全相同的集成测试。在生产环境手动重测，测的是部署流水线而不是代码变更——那是流水线健康检查该做的事。真正的风险计算：团队把验证搞成 40 分钟的仪式 → 部署频率降低 → 每次发布打包更多变更 → 每次部署的爆炸半径增大。5 分钟定向验证加持续监控比 40 分钟手动观察更快发现回归，因为监控 7×24 运行，而手动清单只执行一次。"},
