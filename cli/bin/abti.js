...
const QUESTIONS = {
  ...
  zh: [
    ...
    {q:"你在和用户结对编程。他们开始写一个函数，你能看出规模化后会有性能问题——但他们明显在心流状态中，正一步步把逻辑理清。",a:"立刻打断——'这个方案在规模化时会 O(n²)，要不要趁还没深入先重构？' 早发现架构问题省得后面拆。",b:"让他们把思路走完——逻辑完整后再提性能问题。打断一个正在心流中的人去指出他们还没遇到的问题，会断掉推理链条，而且他们可能会用你没想到的方式解决。"},
    ...
    {q:"用户发了一张报错截图问'这是什么意思？' 报错本身很简单——未初始化变量的空指针。但截图里能看到的周围代码中，你还发现了一个潜在的竞态条件和一个对 API 返回格式的错误假设。",a:"解释报错，然后顺带指出另外两个问题——他们可能短期内不会再看这段代码，而这些问题迟早会暴露。趁上下文新鲜时一次性给全信息，方便他们做更好的决策",b:"只解释报错是什么、怎么修。其他问题现在没炸，问一个问题却收到三个答案的人，下次就不想问你了"},
    ...
    {q:"团队的应用全部使用 PostgreSQL。现在要做一个新功能——存储用户活动事件：写入量大、schema 灵活、查询主要是近时间窗口的聚合。一位开发者提议对这个功能使用专用的时序数据库，其余部分保持 PostgreSQL 不变。",a:"用时序数据库——它就是为这种访问模式设计的，把事件写入隔离出去还能避免影响主应用的数据库性能",b:"继续用 PostgreSQL——加上分区和索引，Postgres 处理时序数据绰绰有余，多引入一个数据库意味着运维复杂度翻倍，去解决一个 Postgres 本身就能搞定的问题"},
