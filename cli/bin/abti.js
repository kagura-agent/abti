...
const QUESTIONS = {
  en: [
    ...
    {q:"Your team is about to ship a payment processing library to production. In the final review, you discover two issues: (1) The retry logic doesn't handle a specific edge case — when the payment gateway returns a 202 (accepted but pending) during a timeout, the library retries and the customer could be charged twice. This affects about 1 in 10,000 transactions under load. (2) The error messages returned to the frontend are generic (\"Payment failed\") — they don't distinguish between \"card declined,\" \"insufficient funds,\" or \"network timeout,\" making it impossible for users to self-diagnose and fix the issue. The PM says you have time to fix ONE before launch; the other goes on the v1.1 roadmap.",a:"Fix the retry edge case. A 1-in-10,000 chance of double-charging is a correctness bug that erodes trust and creates real financial harm, even if most users never hit it. Generic error messages are a UX annoyance that generates support tickets; double charges are a refund nightmare involving the finance team, regulatory risk, and potential chargebacks. Correctness bugs compound as transaction volume grows — at 100,000 transactions per day, that's 10 double charges daily. The error messages are embarrassing but nobody loses money. You don't ship known financial bugs to production when the alternative is a slightly worse error UX for a few weeks.",b:"Fix the error messages. Every single failed payment — not 1 in 10,000 but ALL of them — shows a useless generic message. Users who could fix the problem themselves (\"update your expired card\") instead contact support or abandon the purchase entirely. The retry edge case is real but the probability is low and can be mitigated with an idempotency key on the gateway side while you build the proper fix. Generic errors affect 100% of failure cases; the retry bug affects 0.01% of all transactions. Support ticket volume, user frustration, and conversion loss from bad error messages will dwarf the financial impact of the rare double-charge, which you can also detect and auto-refund in the monitoring layer."},
    {q:"Your team maintains a tech debt register — a document listing known shortcuts, workarounds, and deferred improvements. During this sprint, you took three shortcuts to hit the deadline: (1) Hardcoded a feature flag check instead of using the flag service (saves 30 minutes of integration), (2) Skipped input validation on an internal-only admin endpoint (saves 2 hours of edge case handling), (3) Used a naive O(n²) algorithm for a batch job that runs once per night on ~500 records (saves 4 hours of implementing the efficient version).",a:"Document all three with estimated fix timelines — the hardcoded flag before the next flag-dependent feature, the admin validation before the endpoint goes external, and the O(n²) algorithm before the dataset grows past 5K records (~10 months at current rate). The tech debt register's value comes from completeness — undocumented debt is invisible debt, and invisible debt can't be prioritized against feature work. If only the \"obviously bad\" shortcuts get logged, the register gives a false picture of system health. The 30-minute hardcoded flag is trivial NOW but won't be remembered when someone wonders why the flag service shows one value while the code behaves differently six months from now.",b:"Document only the admin validation gap (#2). The hardcoded flag (#1) will surface naturally the next time someone touches flag logic — \"why is this hardcoded?\" — and takes 30 minutes to fix. The O(n²) algorithm (#3) runs on 500 records in under a second; it's not debt, it's a reasonable implementation for the current scale, and the register entry will be stale long before the dataset approaches 5K. A tech debt register that lists every trivial shortcut becomes a guilt list nobody reads. When everything is \"debt,\" nothing is prioritized. The admin endpoint is the only one with real risk (no input validation on a mutation endpoint) and no natural trigger to fix it — that's what the register is for."},
    {q:"You're opening a pull request that fixes a subtle date-handling bug in the billing system. The timezone offset was applied before the midnight UTC rollover, causing incorrect charge dates for customers outside UTC. The fix is a 3-line change in the date conversion utility. The bug has been in production for 4 months and affected roughly 200 invoices totaling $12K in billing discrepancies. You've confirmed the fix with unit tests and manually verified against 5 affected invoices. A data migration to correct the 200 affected invoices will be a separate follow-up PR.",a:"Write a comprehensive PR description: root cause analysis explaining the midnight rollover edge case, the 3-line fix with inline commentary on why each change matters, your testing approach (unit tests plus manual verification against 5 real invoices), full impact assessment (200 invoices, $12K, 4-month window), and a linked tracking issue for the data migration. A 3-line fix for a 4-month production bug affecting real revenue deserves a PR that serves as the canonical incident record — when someone investigates 'what happened with the Q1 billing discrepancies' in 6 months, this PR should answer every question without requiring archaeology across Slack threads and incident channels.",b:"\"Fixes date conversion for non-UTC timezones. Offset applied before midnight rollover, causing incorrect charge dates for ~200 invoices over 4 months. Tests added, verified against affected invoices. Data migration tracked in #XXX.\" The fix is 3 lines and the diff is self-documenting. Root cause narrative and impact analysis belong in the incident report, not duplicated in the PR. Reviewers need to verify the code change, not read a post-mortem. The team's review velocity depends on descriptions being scannable — a 400-word description for a 3-line change normalizes essay-length PRs, and that culture makes code review feel like homework instead of a quick sanity check."},
  ...
  zh: [
    ...
    {q:"你的团队即将上线一个支付处理库。最终审查时发现了两个问题：(1) 重试逻辑没有处理一个特定边界情况——当支付网关在超时期间返回 202（已接受但待处理）时，库会重试，客户可能被重复扣款。这大约影响万分之一的交易。(2) 返回给前端的错误信息是通用的（\"支付失败\"）——没有区分\"卡被拒绝\"、\"余额不足\"或\"网络超时\"，用户无法自行诊断和解决问题。PM 说上线前只有时间修一个；另一个排到 v1.1。",a:"修复重试边界情况。万分之一的重复扣款概率是一个正确性 bug，会侵蚀信任并造成真实的财务损害，即使大多数用户永远不会遇到。通用错误信息是 UX 层面的不便，会产生工单；重复扣款则是涉及财务团队、合规风险和退单的噩梦。正确性 bug 随交易量增长而放大——每天 10 万笔交易意味着每天 10 笔重复扣款。错误信息令人尴尬但没人因此损失金钱。当替代方案只是几周内稍差一点的错误 UX 时，你不应该带着已知的财务 bug 上线。",b:"修复错误信息。每一笔失败的支付——不是万分之一而是所有——都显示一条无用的通用消息。原本能自己解决问题的用户（\"更新你过期的银行卡\"）转而联系客服或直接放弃购买。重试边界情况确实存在，但概率很低，且可以在网关端用幂等键临时缓解。通用错误影响 100% 的失败场景；重试 bug 影响 0.01% 的所有交易。工单量、用户挫败感和因错误信息不明导致的转化损失，会远远超过偶发重复扣款的财务影响——而且重复扣款也可以通过监控层检测并自动退款。"},
    {q:"你的团队维护着一个技术债务登记表——记录已知的捷径、变通方案和延期改进。这个迭代中你们为了赶截止日期走了三个捷径：(1) 硬编码了特性开关检查，没用开关服务（省了 30 分钟集成时间），(2) 跳过了一个仅内部使用的管理端点的输入验证（省了 2 小时的边界情况处理），(3) 对一个每晚运行一次、处理约 500 条记录的批处理作业使用了 O(n²) 的朴素算法（省了 4 小时的高效算法实现）。",a:"三个都记录，附上预估修复时间——硬编码开关在下一个依赖开关的功能之前修复，管理端点验证在端点对外开放之前修复，O(n²) 算法在数据集增长到 5K 条之前修复（按当前增速约 10 个月）。技术债务登记表的价值在于完整性——未记录的债务是隐形债务，隐形债务无法与功能开发竞争优先级。如果只有「明显有问题」的捷径才被记录，登记表会给出系统健康状况的虚假图景。30 分钟的硬编码开关现在看起来微不足道，但六个月后有人发现开关服务显示一个值而代码行为不一致时，不会有人记得这回事。",b:"只记录管理端点的验证缺失（#2）。硬编码开关（#1）会在下次有人碰到开关逻辑时自然浮出水面——「为什么这里是硬编码的？」——30 分钟就能修好。O(n²) 算法（#3）跑 500 条记录用时不到一秒；这不是债务，而是对当前规模合理的实现，登记表条目会在数据集远未接近 5K 之前就过时。一个把每个微小捷径都列上的技术债务登记表会变成一份没人看的愧疚清单。当一切都是「债务」时，什么都不会被优先处理。管理端点是唯一有真实风险（对修改操作的端点不做输入验证）且没有自然触发修复机制的——这才是登记表存在的意义。"},
