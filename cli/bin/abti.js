...
const QUESTIONS = {
  en: [
    ...
    {q:"You're the tech lead estimating a 6-week feature project for your team of four engineers. The PM needs a timeline for the roadmap presentation next week. You've done two similar projects before — one took 5 weeks, the other took 8 weeks. The main uncertainty is the third-party payment API integration; their documentation looks comprehensive but you haven't written code against it yet.",a:"Provide a week-by-week breakdown: \"Week 1-2: API design and data model (low risk). Week 3-4: Payment API integration and core logic (medium risk, 3-day buffer included). Week 5: Testing and edge case handling. Week 6: Hardening, performance tuning, and staged rollout. Key checkpoint: end of Week 2 we'll know if the payment API behaves as documented. If it doesn't, weeks 3-4 may slip by 3-5 days — we'd flag that at the checkpoint.\" A structured plan surfaces hidden assumptions and creates accountability checkpoints. When Week 2's checkpoint reveals an issue, the PM can adjust scope or staffing proactively. Giving a range without decomposition delegates uncertainty management to the PM, who has less technical context to know what might go wrong and when.",b:"Give a calibrated range with confidence: \"5-7 weeks at 80% confidence. The swing factor is the payment API integration — if it matches their docs, 5 weeks; if it has undocumented quirks or rate limits, add 2-3 weeks. I'll have a much tighter estimate after 2 days of spike work on the API.\" Detailed week-by-week breakdowns for a 6-week project create false precision — the specific activities in weeks 4-6 depend entirely on what you discover in weeks 1-3. A precise-looking Gantt chart doesn't reduce uncertainty, it hides it behind calendar dates. The PM needs to know the confidence interval and the key risk factor, not a fiction of predictable weekly outputs."},
    {q:"Your team maintains a tech debt register — a document listing known shortcuts, workarounds, and deferred improvements. During this sprint, you took three shortcuts to hit the deadline: (1) Hardcoded a feature flag check instead of using the flag service (saves 30 minutes of integration), (2) Skipped input validation on an internal-only admin endpoint (saves 2 hours of edge case handling), (3) Used a naive O(n²) algorithm for a batch job that runs once per night on ~500 records (saves 4 hours of implementing the efficient version).",a:"Document all three with estimated fix timelines — the hardcoded flag before the next flag-dependent feature, the admin validation before the endpoint goes external, and the O(n²) algorithm before the dataset grows past 5K records (~10 months at current rate). The tech debt register works only if it's comprehensive — undocumented debt is invisible debt, and invisible debt can't be prioritized against feature work. The 30-minute flag fix feels trivial, but that's exactly the kind of thing that gets forgotten when the next sprint starts: \"it's only 30 minutes\" becomes \"wait, we have TWO hardcoded flags now?\" three sprints later.",b:"Fix the hardcoded flag right now (it's 30 minutes — shorter than writing and maintaining a register entry for it), and document only the admin validation gap (#2). The O(n²) algorithm (#3) runs on 500 records in under a second; it's an appropriate implementation for the current scale, and a register entry predicting \"fix before 5K records in 10 months\" will be stale and forgotten long before it's relevant. A tech debt register that lists every shortcut regardless of severity becomes a noise-filled checkbox exercise — when 2 out of 3 entries are trivial, the genuine risk (an unvalidated mutation endpoint) gets the same visual weight as a 30-minute flag fix. Keep the register as a high-signal tool for real risks that have no natural trigger to surface them."},
  ...
  zh: [
    ...
    {q:"你的团队维护着一个技术债务登记表——记录已知的捷径、变通方案和延期改进。这个迭代中你们为了赶截止日期走了三个捷径：(1) 硬编码了特性开关检查，没用开关服务（省了 30 分钟集成时间），(2) 跳过了一个仅内部使用的管理端点的输入验证（省了 2 小时的边界情况处理），(3) 对一个每晚运行一次、处理约 500 条记录的批处理作业使用了 O(n²) 的朴素算法（省了 4 小时的高效算法实现）。",a:"三个都记录，附上预估修复时间——硬编码开关在下一个依赖开关的功能之前修复，管理端点验证在端点对外开放之前修复，O(n²) 算法在数据集增长到 5K 条之前修复（按当前增速约 10 个月）。技术债务登记表只有完整才有价值——未记录的债务是隐形债务，隐形债务无法与功能开发竞争优先级。30 分钟的开关修复感觉微不足道，但这恰恰是下个 sprint 开始时就会被遗忘的那种事：「只要 30 分钟」三个 sprint 后变成「等等，我们现在有两个硬编码开关了？」",b:"现在就修掉硬编码开关（30 分钟——比写一个登记表条目再维护它还短），只记录管理端点的验证缺失（#2）。O(n²) 算法（#3）跑 500 条记录用时不到一秒；这是对当前规模合适的实现，一个预测「10 个月后数据到 5K 条前修复」的登记表条目，会在真正相关之前就过时被遗忘。一个不分轻重把每个捷径都列上的技术债务登记表会变成充满噪音的打钩练习——当三个条目里有两个是琐碎的，真正的风险（一个未验证的修改操作端点）就和 30 分钟的开关修复获得同样的视觉权重。让登记表成为一个高信噪比的工具，专门记录没有自然触发机制的真实风险。"},
