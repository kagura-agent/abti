...
const QUESTIONS = {
  en: [
    ...
    {q:"You're adding a new \`lastActive\` timestamp field to your API's user profile endpoint. The existing response already includes 3 date fields that each use a different format: \`createdAt\` returns ISO-8601 (\"2024-01-15T08:30:00Z\"), \`updatedAt\` returns a Unix timestamp (1705302600), and \`birthDate\` returns a human-readable string (\"January 15, 2024\"). This inconsistency has been on the team's known-issues page for 5 months — documented but never prioritized. Your new \`lastActive\` field will be in ISO-8601. The endpoint is consumed by the web frontend, the iOS app, and two third-party integrations via a published API contract.",a:"Standardize all date fields to ISO-8601 while adding \`lastActive\` — shipping a fourth format variation next to three existing inconsistent ones makes the problem actively worse. Every new API consumer has to discover through trial and error which fields use which format, and every inconsistency is a parsing bug waiting to happen. Five months on a known-issues list with no action means it'll never get prioritized on its own — it'll just accumulate more workarounds. A versioned endpoint bump (v2) or a format query parameter gives clients a migration path without breaking existing integrations immediately. You already understand the full response schema from adding \`lastActive\`; someone else picking this up later starts from scratch. One coordinated change is less disruptive than letting the inconsistency grow until a breaking migration becomes unavoidable anyway.",b:"Add \`lastActive\` in ISO-8601, don't touch the other three — 'standardize while you're here' means pushing a breaking change to four consumers (web, iOS, two third-party integrations under API contract) because you noticed something suboptimal while adding a field. The team documented this 5 months ago and didn't prioritize it — that's not negligence, it's a cost-benefit decision you're overriding without the context of why. Third-party integrations parse \`updatedAt\` as an integer; changing it to a string breaks their code. A 'versioned endpoint' or 'format parameter' sounds like a clean solution until you account for: updating API documentation, coordinating deprecation timelines with external partners, supporting both formats during migration, and QA across all four clients. Your PR becomes a cross-team coordination project instead of a field addition. Add your field in the correct format — that sets the precedent for all future fields — and let the broader standardization go through proper API review with stakeholder alignment."},
    ...
    {q:"A product manager asks you (engineering lead) to explain why a feature they expected in 3 days will actually take 3 weeks. You need to reply in a way that gets the right outcome \u2014 the PM understanding the timeline and being able to communicate it upward. Your team\u2019s sprint planning is transparent: the PM has access to your task tracker and attends standups.",a:"Write a detailed task breakdown: API design (2 days), new payment flow implementation (3 days), refund and partial-payment edge cases (2 days), PCI compliance security review (3 days with queue), cross-provider integration testing (2 days), documentation (1 day), buffer for discovered issues (2 days). This gives the PM exactly what they need to defend the timeline to their stakeholders without coming back to you. It also surfaces specific tradeoff points \u2014 maybe \u2018skip refund edge cases for v1\u2019 saves a week and that\u2019s acceptable. Vague explanations like \u2018it\u2019s more complex than it looks\u2019 invite skepticism and repeated why-conversations. A task-level breakdown converts a negotiation about time into a conversation about scope, which is the productive discussion to have.",b:"Explain the critical constraint: \u2018The 3-day estimate assumed reusing our existing payment flow, but this feature routes through our PCI-compliant processing zone which requires a security review (3-day queue) and a separate integration certification. That dependency is the core driver \u2014 even if we parallelized everything else perfectly, the review gate sets the floor.\u2019 Give the PM the structural reason the timeline is what it is, rather than a task list that invites line-item negotiation. A detailed breakdown in this context leads to \u2018can we skip testing?\u2019 and \u2018do we really need documentation?\u2019 \u2014 questions that sound reasonable but have only one right answer. The PM\u2019s actual need is to explain the timeline credibly to their stakeholders, and a clear structural constraint (\u2018external security review\u2019) does that better than fifteen line items they\u2019ll be asked to justify individually."},
  ...
  zh: [
    ...
    {q:"你正在给 API 的用户资料端点添加一个新的 \`lastActive\` 时间戳字段。现有的响应中已经有 3 个日期字段，每个都使用不同的格式：\`createdAt\` 返回 ISO-8601（\"2024-01-15T08:30:00Z\"），\`updatedAt\` 返回 Unix 时间戳（1705302600），\`birthDate\` 返回人类可读字符串（\"January 15, 2024\"）。这个不一致性已经在团队的 known-issues 页面上挂了 5 个月——记录了但从未被列入优先级。你的新 \`lastActive\` 字段将使用 ISO-8601。该端点被 web 前端、iOS 应用和两个通过发布的 API 合约接入的第三方集成所使用。",a:"在添加 \`lastActive\` 的同时将所有日期字段标准化为 ISO-8601——在三个已经不一致的格式旁边再加一个第四种变体，会让问题恶化。每个新的 API 消费者都要通过试错来发现哪个字段用哪种格式，每一处不一致都是一个等待触发的解析 bug。在 known-issues 列表上挂了 5 个月没有行动，意味着它永远不会被自主排优先级——只会积累更多临时方案。版本化端点升级（v2）或格式查询参数可以给客户端一个迁移路径，不会立即破坏现有集成。你已经因为添加 \`lastActive\` 而理解了完整的响应 schema；别人以后来接手得从头开始。一次协调的变更比让不一致性持续增长直到破坏性迁移不可避免要好。",b:"用 ISO-8601 添加 \`lastActive\`，不动其他三个——「顺手标准化」意味着因为你在加字段时注意到了不够理想的地方，就向四个消费者（web、iOS、两个受 API 合约约束的第三方集成）推送破坏性变更。团队 5 个月前记录了这个问题但没有排优先级——那不是疏忽，而是一个你在不了解其原因的情况下要推翻的成本收益决定。第三方集成把 \`updatedAt\` 当整数解析；改成字符串会破坏他们的代码。「版本化端点」或「格式参数」听起来很干净，但你要考虑：更新 API 文档、与外部合作方协调弃用时间线、在迁移期间同时支持两种格式、以及在所有四个客户端上做 QA。你的 PR 从一个字段添加变成了一个跨团队协调项目。用正确的格式添加你的字段——这为所有未来字段树立了先例——然后让更广泛的标准化通过正式的 API 评审和干系人对齐来推进。"},
    ...
    {q:"产品经理问你（技术负责人）为什么一个他们预期 3 天的功能实际需要 3 周。你需要用让 PM 理解并能向上汇报的方式回复。你们团队的 sprint 规划是透明的：PM 有任务追踪器权限并参加站会。",a:"写一份详细的任务分解：API 设计（2 天），新支付流程实现（3 天），退款和部分支付边缘情况（2 天），PCI 合规安全审查（3 天排队），跨支付商集成测试（2 天），文档（1 天），发现问题的缓冲时间（2 天）。这给了 PM 向上游利益相关者解释时间线所需的一切，不用再回来找你。同时也暴露了具体的取舍点——也许'v1 跳过退款边缘情况'能省一周而且可以接受。含糊的解释如'比看起来复杂'会招来质疑和反复追问。任务级分解把关于时间的谈判变成关于范围的对话，这才是有生产力的讨论。",b:"解释关键约束：'3 天的预估假设复用现有支付流程，但这个功能需要经过我们的 PCI 合规处理区域，需要安全审查（3 天排队）和单独的集成认证。这个依赖是核心驱动因素——即使我们完美并行其他所有工作，审查关卡决定了时间下限。'给 PM 时间线背后的结构性原因，而不是一个会引发逐行砍价的任务列表。在这个语境下的详细分解会导致'能跳过测试吗？'和'真的需要文档吗？'——听起来合理但只有一个正确答案的问题。PM 真正需要的是能向利益相关者可信地解释时间线，而一个清晰的结构性约束（'外部安全审查'）比十五个他们会被逐一追问的行项目更有效。"},
