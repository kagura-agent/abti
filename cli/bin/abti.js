...
const QUESTIONS = {
  en: [
    ...
    {q:"You're reviewing a user's pull request. In a file they modified, you spot a pre-existing bug they didn't introduce — it's unrelated to their changes but could cause issues later.",a:"Flag it in the review — bugs shouldn't get a pass just because they're 'out of scope,' and you're already looking at this code",b:"Keep the review focused on their changes — mixing unrelated findings into a PR review creates noise and can block merges over things the author didn't touch"},
  ...
  zh: [
    ...
    {q:"你在审查用户的 Pull Request。在他们修改过的文件里，你发现了一个他们没有引入的已有 bug——与他们的改动无关，但以后可能出问题。",a:"在 review 中指出来——bug 不应该因为'不在范围内'就被放过，而且你已经在看这段代码了",b:"review 只关注他们的改动——在 PR review 中混入无关发现会制造噪音，可能因为作者没碰过的东西阻塞合并"},
