...
const QUESTIONS = {
  en: [
    ...
    {q:"After a 4-hour outage caused by a misconfigured load balancer, you're writing the postmortem. Investigation clearly shows the root cause: a config change deployed without the usual review process because the engineer was handling it during an on-call emergency. During investigation, you also discovered that monitoring for this service has blind spots — it didn't alert until users reported the issue 45 minutes in. The monitoring gap didn't cause the outage; it only delayed detection.",a:"Document both the root cause AND the monitoring gap. The postmortem's purpose is to prevent future incidents, and the monitoring blind spot is a latent risk that will delay detection of the NEXT incident regardless of cause. Keeping scope narrow to \"what caused THIS outage\" misses the systemic improvement opportunity. An incident is a rare window of organizational attention — surface every finding while people care. If you file the monitoring gap as \"a separate ticket,\" it dies in a backlog. Three months from now when the next outage goes undetected for an hour, the postmortem will ask \"why didn't we know about the monitoring gap?\" and the answer will be \"we did, but we decided it was out of scope.\"",b:"Document only the root cause — config change without review — and its remediation. Adding the monitoring gap dilutes the narrative and makes action items ambiguous. The team will debate whether to prioritize the review process fix or the monitoring fix, and do neither well. A focused postmortem with one clear root cause, one clear fix, and one clear owner gets implemented. A comprehensive postmortem with multiple findings gets discussed for a week and produces a Jira graveyard. The monitoring gap is real, but mixing it into this postmortem reduces the probability that EITHER improvement actually ships. File it separately with its own priority discussion, its own owner, and its own deadline — that's not burying it, that's giving it the dedicated attention it deserves instead of making it a footnote in someone else's incident."},
    {q:"You're setting up monitoring alerts for an e-commerce order processing service. Average processing time is 200ms, p95 is 400ms, p99 is 800ms. The service handles 10,000 orders per hour during peak. You need to set the latency alert threshold and window.",a:"Set threshold at p95 (450ms) with a 5-minute sustained window. This catches degradation early — when latency shifts from \"normal\" to \"trending bad\" — before it noticeably affects user experience. Yes, it will fire occasionally during deploy windows and batch processing, probably 2-3 times per week. Those are the cost of early detection. You'd rather investigate a false alarm in 10 minutes than miss the first 30 minutes of a real degradation because your threshold only fires when things are obviously broken. At 10,000 orders per hour, 30 minutes of undetected degradation means 5,000 orders processed slowly with no one looking. Early alerts let you catch a memory leak at \"slightly slow\" instead of \"falling over.\"",b:"Set threshold at 1500ms (roughly 2x p99) with a 10-minute sustained window. An alert that fires when nothing is actually wrong trains the team to ignore alerts. After a month of 2-3 false alarms per week during deploys and batch jobs, the on-call engineer's response becomes \"probably another deploy, snooze.\" When a real degradation happens, it sits in a notification graveyard. The 10-minute window means transient spikes resolve before triggering, but sustained problems — memory leaks, bad query plans, downstream timeouts — always fire. An alert system's value isn't measured by how early it detects problems; it's measured by the probability that a human actually investigates when it fires. A p95 alert with 60% investigation rate catches fewer real incidents than a 2x-p99 alert with 100% investigation rate."},
  ...
  zh: [
    ...
    {q:"一次由负载均衡器配置错误引起的 4 小时故障后，你在写事后复盘。调查清楚地显示了根因：一位工程师在处理 on-call 紧急情况时，跳过了常规 review 流程直接部署了配置变更。调查过程中你还发现该服务的监控存在盲区——直到用户报告问题 45 分钟后才触发告警。监控缺陷没有导致故障，只是延迟了发现。",a:"同时记录根因和监控缺陷。事后复盘的目的是防止未来的故障，而监控盲区是一个潜在风险，无论下次故障原因是什么都会延迟发现。把范围限制在「导致本次故障的原因」会错失系统性改进的机会。故障是组织注意力集中的稀缺窗口——趁大家还关心时把所有发现都摆出来。如果把监控缺陷「另开工单」，它会烂在 backlog 里。三个月后下一次故障因为监控盲区被延迟发现一小时，复盘会问「为什么我们不知道监控有问题？」，答案是「我们知道，但当时决定不在范围内。」",b:"只记录根因——跳过 review 的配置变更——以及对应的改进措施。加入监控缺陷会冲淡叙事，让行动项变得模糊。团队会争论是先修 review 流程还是先修监控，结果两个都做不好。聚焦的复盘，一个清晰的根因、一个清晰的修复、一个清晰的负责人，才能真正落地。面面俱到的复盘讨论一周，产出一堆 Jira 僵尸票。监控缺陷确实存在，但混进这份复盘会降低任何一项改进实际交付的概率。单独立项，有自己的优先级讨论、自己的负责人、自己的截止日——这不是搁置，是给它应有的独立关注，而不是让它成为别人故障里的一个脚注。"},
    {q:"你在为一个电商订单处理服务配置监控告警。平均处理时间 200ms，p95 400ms，p99 800ms。高峰期每小时处理 10,000 单。你需要设定延迟告警的阈值和时间窗口。",a:"阈值设在 p95（450ms），5 分钟持续窗口。这能在延迟从「正常」转向「趋势变差」时及早发现，在明显影响用户体验之前。确实会在部署窗口和批处理期间偶尔触发，大概每周 2-3 次。这是早期发现的代价。你宁愿花 10 分钟排查一次误报，也不愿因为阈值设得只在「明显出事」才响而错过真正退化的前 30 分钟。每小时 10,000 单，30 分钟未检测到的退化意味着 5,000 笔订单在没人关注的情况下被缓慢处理。早期告警让你在「稍微慢了」而不是「快挂了」的时候抓住内存泄漏。",b:"阈值设在 1500ms（约 p99 的 2 倍），10 分钟持续窗口。一个在没出问题时也会响的告警会训练团队忽略告警。一个月下来每周 2-3 次因为部署和批处理触发的误报后，on-call 工程师对延迟告警的条件反射就变成了「大概又是部署，静音」。真正出问题时，通知淹没在历史记录里。10 分钟窗口意味着瞬态尖峰在触发前就恢复了，但持续性问题——内存泄漏、糟糕的查询计划、下游超时——一定会触发。告警系统的价值不在于多早发现问题，而在于响起时人类实际去排查的概率。p95 告警 60% 的排查率，实际捕获的真实事故比 2x-p99 告警 100% 排查率更少。"},
