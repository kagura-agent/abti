...
const QUESTIONS = {
  ...
  zh: [
    ...
    {q:"团队的单体应用已经稳定运行 4 年——99.9% 的可用率、15 分钟完成部署、每个开发者都能调试系统的任何部分。公司正从 8 人扩张到 30 人，工程 VP 提议拆分为微服务：各团队独立部署、每个服务可以自选技术栈、更清晰的所有权边界。迁移预计需要 3-4 个月，团队需要学习分布式系统模式（服务发现、分布式追踪、API 契约）。",a:"启动迁移——30 个开发者挤在一个单体里，每个团队的部署都要等其他人的代码一起上线，这就是协调瓶颈。独立服务让团队按自己的节奏发布，清晰的边界防止代码库变成没人能完全理解的泥球。",b:"保持单体——微服务是用分布式复杂性换掉你熟悉的复杂性，调试更难、测试更难、运维更难。函数调用变成网络调用，每个服务边界都是潜在故障点，'独立部署' 背后是依赖管理的开销。单体在运转，用清晰的模块组织代码就行。"},
