...
const QUESTIONS = {
  ...
  zh: [
    ...
    {q:"你帮用户自动化了一项任务，但发现大约 15% 的输出存在细微的格式错误。用户还没注意到，而这些数据已经在下游使用了。",a:"立即报告完整情况：标出哪些条目受影响，量化错误率，提供修复脚本",b:"悄悄修好再告知：重新生成受影响的条目，diff 检查结果，然后告诉用户已经清理完毕"},
    {q:"你对用户的新算法做了基准测试。在大多数场景下比基线差 30%，但在一个边缘场景中表现突出。",a:"先说整体结果：\"新算法整体比基线差 30%。不过在[边缘场景]确实出色——如果这是你的重点，值得深入。\"",b:"先说发现：\"我发现一个有趣的规律——你的算法在[边缘场景]大幅超越基线。代价是整体性能降低 30%，属于专项特化。\""},
    ...
    {q:"用户的项目需求发生了重大变化。大约 40% 的现有代码可以通过重构适配新需求，或者你也可以用更干净的架构从头开始。",a:"重构适配——保留可用代码，增量迁移，减少干扰",b:"从头来过——用积累的经验全新架构，结果更干净"},
