...
const QUESTIONS = {
  ...
  zh: [
    ...
    {q:"你在凌晨3点修复了一个生产故障——支付服务因数据库连接池耗尽宕机了43分钟，无数据丢失。现在凌晨4点，CEO（非技术背景）、工程VP和其他工程师都在一个共享的Slack频道等待你的更新。",a:"写一份结构化的事故摘要：时间线（02:17 告警、02:23 定位到连接池耗尽、02:31 增加连接限制、03:00 确认所有交易恢复正常处理），用通俗语言解释根因（流量高峰期同时打开过多连接压垮了数据库），客户影响（支付处理不可用43分钟，约340笔失败交易，恢复后全部自动重试成功），以及后续步骤（今天安排正式事后复盘，已收紧监控告警）。CEO、VP和工程师各自需要不同的信息——但一份结构化摘要能同时满足三方：CEO看到影响和解决情况，VP看到时间线和流程，工程师看到技术原因和后续步骤。凌晨4点的一条完整消息，能避免明天早上三个不同的「到底怎么回事？」讨论串，并成为后续正式事后复盘的基础记录。",b:"「支付服务03:00恢复。宕机43分钟，根因：流量高峰期数据库连接池耗尽。无数据丢失——340笔失败交易已全部自动重试成功。今天进行事后复盘。」凌晨4点，频道只需要三个信息：修好了吗，影响多大，接下来怎么办。在4点写时间线、通俗根因解释和后续步骤，等于在事后复盘之前先写了一遍事后复盘——那不是正式复盘干的事吗。结构化摘要还会形成惯例，让每个值班人员都被期望在睡觉前写出一份精美报告，这会把on-call工程师累垮。把关键事实发出去，去睡觉，等不再靠肾上腺素和咖啡撑着的时候再写详细分析。"},
    {q:"团队的订单处理服务从合作方集成接收订单事件。每个事件载荷包含 34 个字段——你的处理逻辑只读取其中 6 个（order_id、amount、currency、status、timestamp、customer_tier）。合作方 API 在过去一年中两次更改了载荷结构（字段重命名、你的代码未读取的字段类型变更）。你正在为这个集成定义输入校验策略。",a:"按合作方的文档 schema 校验全部 34 个字段。合作方下次把 shipping.region 改名为 shipping.zone 时，你的校验在第一个事件就能捕获 schema 偏移——而不是 4 个月后有人查分析报告中的物流数据才发现是乱的。全量校验每个事件约多 3ms，相当于一个实时的 schema 合规监控。28 个字段不校验就存下来，指望合作方永远不会在你没看的字段上搞破坏性变更——而他们的历史记录已经证明了相反——这就是静默数据腐败在看起来健康的系统中悄悄积累的方式。",b:"只校验你代码实际读取的 6 个字段。载荷全量存储但不对未查询的字段强制 schema 规则。校验 28 个你不读的字段意味着把服务可用性绑定到合作方的完整 schema 上——他们下次改个字段名就会触发每天 1 万条校验错误，而且是你从未使用过的数据。这不是尽职，是伪装成严谨的脆弱性。一个在每次无关 schema 变更时都报警的校验规则，训练的是团队忽略报警。等到你写代码读某个字段时再校验它——不要投机性地校验，不要以可用性为代价。"},
