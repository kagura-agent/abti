...
const QUESTIONS = {
  en: [
    ...
    {q:"You've inherited a complex 800-line module that implements a priority queue with custom eviction policies for a caching layer. The code has zero comments or documentation. It's well-structured — meaningful variable names, small functions, clear abstractions — but the business logic behind the eviction policies (why certain priorities expire at different rates, why the rebalancing threshold is 0.7, why expired items get a 30-second grace period) isn't explained anywhere. The engineer who wrote it left the company 6 months ago. A new team member starts in 2 weeks and this module is their first assignment.",a:"Add inline comments throughout the module explaining the 'why' behind each business rule and algorithmic choice. Document why the rebalancing threshold is 0.7 (tested values from 0.5 to 0.9 under production load — 0.5 caused excessive rebalancing during traffic bursts, 0.9 let the queue degrade before correcting), why expired items get a 30-second grace period (downstream consumers poll every 25 seconds, so immediate eviction causes unnecessary cache misses during the polling gap), and the priority tiebreaking logic. These business reasons aren't derivable from the code no matter how clean the variable names are — reading GRACE_PERIOD_MS = 30000 tells you the value but not why it's 30 seconds instead of 10 or 60. Comments do require maintenance when code changes, but business context that exists only in a departed engineer's memory decays faster than any comment ever could.",b:"Write a standalone design document (1-2 pages) covering the module's architecture, the rationale behind each eviction policy, and a reference table of tuned constants with their tested alternatives and production context. Keep the code comment-free — the clean structure already communicates the 'what' and 'how,' and distributing explanations across 800 lines creates a maintenance surface where every code change demands a matching comment update that rarely happens. A centralized doc is easier to review for accuracy, update in one place when policies change, and link from the onboarding guide. For the new team member, a 2-page design document they read in 15 minutes before touching the code provides more useful context than 60 comments they encounter one at a time while navigating unfamiliar abstractions without the big picture. Document the forest; let the clean code document the trees."},
