...
const QUESTIONS = {
  en: [
    ...
    {q:"Your authentication service handles login failures: wrong password, email not found, account locked, expired password. Currently all four return a generic authentication_failed error code. The product team wants distinct error codes so the client can show targeted UI messages. The support team handles 30 'can't log in' tickets per week — 40% are locked accounts who could self-resolve with the right message.",a:"Return all four distinct error codes: invalid_credentials, account_not_found, account_locked, password_expired. Right now locked-account users see a generic 'login failed' and retry five or more times before calling support. Distinct codes let the client show 'Account locked — contact admin,' cutting roughly 40% of support tickets — that's 12 real tickets per week. The enumeration risk is theoretical given existing defenses: 5 attempts per IP, CAPTCHA after 3, full lockout after 10. Real credential attacks use leaked databases, not login form probing. The 12 weekly tickets from locked accounts are a concrete, measurable cost; the incremental enumeration risk is negligible when leaked databases already provide email lists.",b:"Keep a unified authentication_failed for wrong-password and email-not-found, but add separate codes for account_locked and password_expired — these only confirm already-disabled accounts, so they create no enumeration channel. Separating wrong-password from email-not-found is the actual enumeration vulnerability, and IP rate limiting only partially mitigates it: credential stuffing rotates through thousands of residential proxies. Knowing which emails are registered reduces attack space from O(emails × passwords) to O(passwords). The product UX goal is better served by improving the password reset flow. Three of four error codes gives the product team most of what they want while preserving the one security-critical distinction attackers would exploit — registered versus unregistered."},
