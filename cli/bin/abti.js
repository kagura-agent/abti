...
const QUESTIONS = {
  en: [
    ...
    {q:"You're establishing testing standards for a new team (8 engineers) working on an e-commerce order processing service. The service handles ~$2M/day in transactions. The team currently has no formal testing policy. You need to define what 'adequately tested' means for PRs to this codebase.",a:"Every public function gets unit tests — success, failure, and edge cases. Mock all external dependencies. Integration tests for every API endpoint. Target 90%+ line coverage enforced in CI (PRs below threshold blocked). Test names document behavior: should_reject_order_when_inventory_insufficient_and_payment_already_captured. Yes, this means writing 3 tests for a function that formats a currency string. But that formatting function will eventually handle JPY (no decimals), BHD (3 decimals), and negative values for refunds — without tests on the simple case first, those edge cases will never get caught. High coverage establishes the habit and infrastructure so when code stops being trivial, tests already exist. Teams without coverage gates inevitably accumulate 'we'll add tests later' debt that compounds until an incident forces a retroactive testing sprint.",b:"Test at the boundary — API endpoint tests that exercise real workflows (place order → check inventory → charge payment → confirm). Unit test only pure business logic with complex branching (discount calculation, tax rules, fraud scoring). No coverage threshold in CI — coverage percentage doesn't measure confidence. A codebase at 95% coverage that tests implementation details breaks on every refactor and teaches developers to write tests that pass rather than tests that catch bugs. Focus testing time on scenarios that would page you at 3 AM: payment double-charges, inventory oversell, order state corruption. Testing formatCurrency('1234.5') returns '$1,234.50' catches nothing a type system and code review wouldn't — but testing 'what happens when payment gateway times out mid-transaction' catches bugs that actually cost $50K. Teams with coverage gates write worse tests because the incentive becomes 'make the number go up' not 'make the system reliable.'"},
  ...
  zh: [
    ...
    {q:"你在为一个新团队（8 名工程师）制定测试标准，该团队负责一个电商订单处理服务。服务每天处理约 200 万美元的交易。团队目前没有正式的测试规范。你需要定义 PR 合入这个代码库时「测试充分」意味着什么。",a:"每个公共函数都要有单元测试——成功、失败和边界情况。Mock 所有外部依赖。每个 API 端点都要有集成测试。CI 中强制 90% 以上行覆盖率（低于阈值的 PR 被阻塞）。测试名称即行为文档：should_reject_order_when_inventory_insufficient_and_payment_already_captured。是的，这意味着为一个格式化货币字符串的函数写 3 个测试。但那个格式化函数终将需要处理 JPY（无小数）、BHD（3 位小数）和退款的负数值——如果简单场景没有测试打底，这些边界情况永远不会被覆盖到。高覆盖率建立的是习惯和基础设施，让代码不再简单时测试已经就位。没有覆盖率门槛的团队必然累积「以后再补测试」的债务，滚雪球般膨胀直到一次事故逼你做追溯性的测试冲刺。",b:"在边界处测试——用 API 端点测试来覆盖真实工作流（下单 → 检查库存 → 扣款 → 确认）。只对有复杂分支的纯业务逻辑做单元测试（折扣计算、税务规则、风控评分）。CI 中不设覆盖率阈值——覆盖率百分比不等于信心。一个 95% 覆盖率但测试实现细节的代码库在每次重构时都会崩溃，而且训练开发者写「能通过的测试」而非「能抓 bug 的测试」。把测试时间集中在凌晨 3 点会叫醒你的场景：重复扣款、超卖、订单状态损坏。测试 formatCurrency('1234.5') 返回 '$1,234.50' 抓不到类型系统和代码审查抓不到的东西——但测试「支付网关在交易中超时会发生什么」能抓到真正值 5 万美元的 bug。有覆盖率门槛的团队写出更差的测试，因为激励变成了「让数字上去」而不是「让系统可靠」。"},
