...
const QUESTIONS = {
  en: [
    ...
    {q:"The user's team deploys their SaaS app with a shell script: SSH into three servers, pull the latest tag, restart services. It takes 4 minutes, the team has done 600+ deploys over 3 years without a single rollback, and new hires can deploy independently on day one. A senior SRE proposes migrating to GitOps with ArgoCD: automated staging deploys on merge, canary production rollouts, instant rollback via git revert. Setup takes 2-3 weeks and requires the team to learn declarative deployment manifests.",a:"Migrate to GitOps — 600 incident-free deploys doesn't mean the process is safe, it means nothing has tested it yet. Canary rollouts catch bad releases before they reach all users, staging validates in a real environment, and git-revert rollback is faster than SSHing under pressure.",b:"Keep the shell script — 3 years and 600 deploys without rollback is the strongest evidence a process works. A 4-minute deploy new hires run on day one is an advantage most teams never achieve. GitOps adds complexity to solve for hypothetical incidents. Simple systems fail less."},
  ...
  zh: [
    ...
    {q:"用户让你 review 一个 200 行的 PR。你发现了一个关键 bug、三处轻微风格不一致、以及两处可以用更新 API 写得稍微更优雅的地方。",a:"六个问题全标出来并说明原因——完整的 review 建立一致标准，帮团队发现潜在模式",b:"bug 标为阻塞，风格简单提一句。API 建议不写——对正常工作的代码吹毛求疵增加 review 疲劳，收益极小"},
    ...
    {q:"用户团队用一个shell脚本部署SaaS应用：SSH连到三台服务器，拉最新tag，重启服务。整个过程4分钟，团队3年来完成了600多次部署从未回滚，新人入职第一天就能独立部署。一位资深SRE提议迁移到ArgoCD的GitOps流程：PR合并后自动部署到预发布环境，生产环境金丝雀发布，通过git revert即时回滚。搭建需要2-3周，团队需要学习声明式部署清单。",a:"迁移到GitOps——600次无事故部署不代表流程安全，只代表还没被真正考验过。金丝雀发布能在坏版本影响所有用户之前拦截，预发布环境在真实环境中验证变更，git revert回滚比紧急SSH进服务器更快。",b:"保持shell脚本——3年600次部署零回滚就是流程可靠的最强证据。4分钟部署、新人第一天就能操作，是大多数团队永远达不到的优势。GitOps增加声明式抽象层，引入新的故障点，用真实复杂性解决假设性事故。简单系统故障更少。"},
