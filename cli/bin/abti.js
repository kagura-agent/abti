...
const QUESTIONS = {
  en: [
    ...
    {q:"You're debugging a flaky test that fails about 1 in 20 CI runs. The test covers a payment processing flow. When it fails, the error is \"assertion timeout: expected payment status to be 'completed' within 5000ms.\" The test has been flaky for 3 weeks and is currently set to auto-retry (so it doesn't block merges). Two other engineers have each spent about 2 hours looking at it without finding the root cause.",a:"Reproduce the failure deterministically before fixing anything. Add timing instrumentation to every async step in the payment flow, inject controlled delays to isolate which step is timing-sensitive, and run the test 500 times locally to collect a statistical profile of where time is spent. Once you can reliably trigger the failure, the fix becomes obvious — and you'll know the fix actually solves the problem rather than just making the race window narrower. Two engineers already spent 2 hours each on quick fixes that didn't work. The pattern of \"guess at the timing issue, bump a timeout, hope it sticks\" is exactly how flaky tests stay flaky for months. Invest 4 hours in understanding the problem completely, get a permanent fix.",b:"The error message says \"timeout waiting for payment status 'completed'\" — so the payment callback is sometimes slower than 5 seconds. Check the test environment's event queue latency (takes 10 minutes). If latency spikes explain the flaky timing, either increase the timeout to 10s or switch to a polling assertion that waits for the status change event rather than a fixed timeout. The test has been \"auto-retry and move on\" for 3 weeks — nobody is actually blocked, and 4 hours of instrumentation for a test-environment timing issue is debugging theater. The two engineers who already spent 2 hours each probably already identified the likely cause; they just didn't want to \"just bump the timeout\" because it feels like a hack. Sometimes the hack is the right fix — the test environment has different performance characteristics than production, and the test should account for that."},
