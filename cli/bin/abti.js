...
const QUESTIONS = {
  en: [
    ...
    {q:"After a 2-hour outage caused by a race condition in the order pipeline, you need to assess how many of the ~3,000 orders processed during that window were affected. The race condition triggers when two orders for the same customer arrive within 200ms — the second order reads stale inventory state.",a:"Build a reconciliation script that replays all 3,000 orders through the fixed pipeline and compares outputs against what was actually produced. Takes 2 days to build. Gives you a definitive answer for every single order — which were correct, which were skipped, which produced wrong results. But the replay harness is itself new untested code running against production data — if the harness has even a subtle bug in how it handles replay sequencing or data dependencies, it could flag correct orders as corrupted (triggering unnecessary refunds and customer notifications) or confirm corrupted orders as correct (missing affected customers entirely). You're judging system A's correctness using system B that has had zero production hours. Any discrepancy the harness reports still needs manual verification to determine whether the original order or the replay is wrong.",b:"Query the event log for same-customer order pairs arriving within the 200ms race window: 18 pairs across 11 customers. Manually verify these 18 against the payment ledger (half a day of work). The remaining 2,964 orders went through a code path where the race condition is structurally impossible — a single-customer order can't race against itself. This approach is faster and directly targeted at the mechanism, but it assumes your root-cause analysis is complete. If there's a second trigger you haven't identified (e.g., a shared inventory counter that isn't customer-scoped), those 2,964 orders remain unchecked. You're betting on the quality of your debugging rather than the completeness of your audit."},
  ...
  zh: [
    ...
    {q:"订单管道中的一个竞态条件导致了 2 小时的故障，你需要评估这段时间内处理的约 3,000 个订单中有多少受到了影响。竞态条件在同一客户的两个订单在 200ms 内到达时触发——第二个订单读取到了过期的库存状态。",a:"编写一个对账脚本，将全部 3,000 个订单在修复后的管道中重放，并与实际产出进行比对。需要 2 天时间来构建。能给出每一个订单的确定性答案——哪些是正确的、哪些被跳过了、哪些产生了错误结果。但重放工具本身就是一段未经测试的新代码，要跑在生产数据上——如果工具在处理重放顺序或数据依赖时有哪怕一个细微的 bug，它可能会把正确的订单标记为损坏（触发不必要的退款和客户通知），或者把损坏的订单确认为正确（完全漏掉受影响的客户）。你在用一个零生产运行时间的系统 B 来判断系统 A 的正确性。工具报告的任何差异仍然需要人工核查，才能判断是原始订单还是重放结果出了错。",b:"查询事件日志，找出在 200ms 竞态窗口内到达的同客户订单对：11 个客户涉及 18 对订单。将这 18 对与支付台账进行人工核对（半天的工作量）。剩余的 2,964 个订单走的是竞态条件在结构上不可能发生的代码路径——单客户订单无法与自己竞争。这个方法更快，直接瞄准了触发机制，但前提是你的根因分析是完整的。如果存在你尚未发现的第二个触发条件（比如一个不以客户为维度的共享库存计数器），那 2,964 个订单就仍然没有被检查。你押注的是自己调试的质量，而非审计的完整性。"},
