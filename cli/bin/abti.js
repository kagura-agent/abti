...
const QUESTIONS = {
  en: [
    ...
    {q:"You're shipping v2.0 of an internal API that 12 engineering teams depend on. The API handles requests correctly 99.9% of the time, but has a known race condition: when two services make concurrent writes to the same resource within a 50ms window, last-write-wins silently drops the first write. This affects roughly 0.1% of requests in production. The proper fix (optimistic locking) is scoped for v2.1, scheduled 6 weeks out. You're writing the v2.0 release notes that will go to all 200+ engineers in the organization.",a:"Document the race condition in the release notes — exact trigger, impact (0.1% of requests, silent data loss), and workaround until v2.1. Engineers need to know failure modes before designing around your API. Some teams may be building workflows where silent data loss is catastrophic. Release notes are what engineers actually read; six weeks is long enough to ship a feature depending on write consistency they don't have.",b:"Add it to the internal known-issues tracker and list \"improved write concurrency\" as a v2.1 highlight. Putting \"silent data loss\" in release notes read by 200+ engineers guarantees escalation meetings and teams blocking adoption even if they never do concurrent writes. The workaround is more complex than the bug for most use cases. The 3 teams that actually hit this can find it in the tracker."},
  ...
  zh: [
    ...
    {q:"你花了 2 天调查客户数据导出偶尔产生损坏文件的问题（影响 0.3% 的导出）。你找到了根因：一个竞态条件，仅在导出超过 2GB 且垃圾回收暂停恰好发生在最终缓冲区刷写时才会触发。修复是 4 行代码，加一个刷写同步屏障。组长让你给团队写一份问题总结。",a:"完整报告：复现步骤、排除的三个假说（磁盘 I/O、网络超时、编码错误）、精确触发条件（>2GB + GC 暂停期间刷写）、同步屏障为何能修复、已知限制（>2GB 的导出增加约 200ms）。排除的假说为团队节省未来调查时间——下次遇到类似问题，不必再花 2 天走死胡同。触发条件记录了如果屏障被移除时问题何时会复发。",b:"'根因：大文件导出缓冲区刷写时的竞态条件。修复：最终写入前加同步屏障。>2GB 的导出增加约 200ms（占总导出量 0.3%）。PR #482。' 排除的假说是你的调查过程，不是团队需要关心的。他们需要知道什么坏了、怎么修的、代价多大。把死胡同塞进报告会降低信噪比——真正的发现淹没在叙述里。如果有人想看调查路径，可以单独问或者看 PR 评论。"},
    {q:"你在发布一个内部 API 的 v2.0 版本，12 个工程团队依赖这个 API。API 在 99.9% 的情况下处理请求正确，但有一个已知的竞态条件：当两个服务在 50ms 窗口内对同一资源发起并发写入时，后写入会静默覆盖先写入的数据。这影响了生产环境约 0.1% 的请求。正确的修复方案（乐观锁）已纳入 v2.1 规划，预计 6 周后发布。你正在写 v2.0 的发版说明，会发给组织内所有 200+ 名工程师。",a:"在发版说明里记录这个竞态条件——精确触发条件、影响（0.1% 请求静默丢数据）、以及临时方案。工程师需要在设计方案前了解 API 的失败模式。有些团队可能在构建静默丢数据会出大问题的功能。发版说明才是工程师真正会看的；六周足够上线一个依赖写入一致性的功能。",b:"记到已知问题跟踪器里，v2.1 亮点列出「优化写入并发处理」。在 200+ 工程师都看的发版说明里写「静默丢数据」，必然引发升级会议和团队拒绝采用——即使他们根本不做并发写入。临时方案对大多数场景比 bug 本身还复杂。真正受影响的那 3 个团队可以在跟踪器里找到。"},
    {q:"用户问你怎么看同事的代码，质量不好。",a:"直接指出具体问题和改进建议",b:"先肯定优点，再温和地建议改进"},
    {q:"你发现并修复了计费服务中一个存在 2 个月的 bug：时区转换错误导致约 200 名国际用户看到的发票时间戳偏移了一小时。发票其他方面完全正确——金额、明细、支付状态都没问题。修复是 3 行代码（硬编码 UTC 偏移 → 时区库），你还写了数据迁移来修正存储的时间戳。",a:"在工程全员频道发一份简短的事故报告——bug 描述、影响（200 用户、仅显示时间戳偏移、无计费错误）、修复方案和根因。即使是小 bug 也值得可见度：其他人的服务可能有同样的硬编码偏移模式，让小问题的公开复盘成为常态，也更容易在大问题时坦诚披露。",b:"修完、跑迁移，写进团队 sprint 总结里。一个只影响显示的时间戳偏移、涉及 200 用户，是常规 bug 修复，不是工程全员的学习事件。给 150 个对计费服务毫无上下文的工程师广播这种事只会制造噪音，让人养成跳过工程频道的习惯。复盘文化在用于揭示系统性模式的事故时才有效——不是每个 3 行补丁都需要。"},
